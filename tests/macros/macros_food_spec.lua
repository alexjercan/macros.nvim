...
describe("Food.from errors", function()
    it("errors on missing unit", function()
        assert.has_error(function()
            Food.from("apple 100")
        end)
    end)

    it("errors on unknown unit", function()
        assert.has_error(function()
            Food.from("apple 100kg")
        end)
    end)

    it("errors on non-numeric amount", function()
        assert.has_error(function()
            Food.from("apple abcg")
        end)
    end)
end)
