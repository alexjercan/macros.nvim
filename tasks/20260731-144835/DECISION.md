...
4. The repo DOES carry a version, in two places - `VERSION` in `macros.lua`
   (the CLI's `--version`) and `version` in `flake.nix` (the package). The
   plan's claim that no version existed was wrong: it grepped only `lua/` and
   `plugin/`. Both are bumped by the release procedure, and the release job
   refuses a tag that disagrees with them, because two hand-maintained
   constants drift silently otherwise.
5. Release notes are GitHub's `--generate-notes`; the CHANGELOG stays the
   curated human record. No extraction script - explicitly rejected by the user
   as overcomplicated.
